...
export function Dashboard() {
  ...
  const voluntarios = React.useMemo(() => {
    try {
  ...
  const dossiers = React.useMemo(() => {
    try {
      const saved = localStorage.getItem('satai_dossiers');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  ...
  // Métricas Dinâmicas
  const totalHoursDonated = React.useMemo(() => {
    const volHours = voluntarios.reduce((acc: number, v: any) => acc + (Number(v.horasMes) || 0), 0);
    const profHours = professionals.reduce((acc: number, p: any) => acc + (Number(p.hoursDonated) || 0), 0);
    return volHours + profHours;
  }, [voluntarios, professionals]);

  const completedAppointments = React.useMemo(() => {
    return appointments.filter((a: any) => a.status === 'completed').length;
  }, [appointments]);

  const pendingTriageCount = React.useMemo(() => {
    return dossiers.filter((d: any) => d.status === 'pending_review').length;
  }, [dossiers]);

  const upcomingList = React.useMemo(() => {
    return appointments.filter((a: any) => a.status === 'upcoming');
  }, [appointments]);

  const inProgressCount = React.useMemo(() => {
    return appointments.filter((a: any) => a.status === 'in_progress').length;
  }, [appointments]);

  const activeRooms = React.useMemo(() => {
    const list = appointments.filter((a: any) => a.status === 'in_progress' || a.status === 'upcoming');
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-3xl font-bold text-teal-700">{totalHoursDonated}h</div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-shadow">
                  <div className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Fila de Triagem (SATAI)</div>
                  <div className="text-3xl font-bold text-amber-600">{pendingTriageCount}</div>
                  <div className="text-xs text-amber-700 mt-2 flex items-center gap-1.5 font-medium bg-amber-50 inline-flex px-2 py-1 rounded-md border border-amber-100/50">
                    ...
                    Casos aguardando revisão técnica
                  </div>
                </div>
              </div>
                      ...
                      {upcomingList.map((apt, idx) => (
                        <div key={apt.id} className="p-5 flex items-center gap-6 hover:bg-slate-50/50 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-base font-semibold text-slate-900 truncate">{apt.patientName}</h4>
                            <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                              ...
                              <span>Agendado ({apt.date})</span>
                            </div>
                          </div>
                          
                          <div>
                            {idx === 0 && apt.type === 'online' ? (
                              <button 
                              ...
                              <button 
                                onClick={() => navigate(`/patients/${apt.patientId || '1'}`)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="flex h-2.5 w-2.5 font-mono">
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
                  ...
                  <div className="text-3xl font-bold text-slate-900 mt-2">{completedAppointments}</div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="flex justify-between items-center bg-stone-50 p-2.5 rounded-lg">
                      ...
                      <span className="font-semibold text-amber-600 font-mono">0 / {activeRooms.length}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3 text-xs">
                    <div className="flex justify-between items-center bg-stone-50 p-2.5 rounded-lg">
                      ...
                      <span className="font-semibold text-teal-700 flex items-center gap-1 font-mono">
                        <UserCheck className="w-3.5 h-3.5 text-teal-650" /> {professionals.filter(p => p.status === 'ACTIVE').length} técnicos
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-stone-50 p-2.5 rounded-lg">
                      ...
                      <span className="font-semibold text-rose-600 font-mono">{pendingTriageCount} pendentes</span>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                      {activeRooms.map((room) => (
                        <tr key={room.id}>
                          <td className="py-3.5 font-mono text-slate-400">{room.id}</td>
                          <td className="py-3.5">{room.professional}</td>
                          <td className="py-3.5">{room.patient}</td>
                          <td className="py-3.5 font-mono">{room.duration}</td>
                          <td className="py-3.5 text-center">
                            <span className="px-2.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-md font-semibold">{room.quality}</span>
                          </td>
                          <td className="py-3.5 text-right text-teal-650">{room.e2ee}</td>
                        </tr>
                      ))}
